...
# python decorator Python function with @function_tool(). The function’s signature and docstring are used to describe the tool to the LLM.
...
toolstemplate = Template(
    template=[
        ...
        UserMessage(content=" solve the calculation expression {{?a}} * {{?b}}? and {{?a}} + {{?b}}?"),
    ],
    tools=tools,
)

# Assume 'template' and 'tool' are defined as above
...
for tool_call in tool_calls:
    ...
    result = multiply.execute(**tool_call.function.parse_arguments())
