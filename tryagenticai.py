...
prompt_template = PromptTemplatingModuleConfig(prompt=v2template, model=v2llm)
module_config = ModuleConfig(prompt_templating=prompt_template)
config = OrchestrationConfig(modules=module_config)
orchestration_service = OrchestrationService(config=config)
result = orchestration_service.run(placeholder_values={"user_query": "Who was the first person on the moon?"})
print(result.final_result.choices[0].message.content)
...
v3template = Template(
    ...
    defaults={"user_query": "Who was the first prime minister of india"}
)


prompt_template = PromptTemplatingModuleConfig(prompt=v3template, model=v2llm)
module_config = ModuleConfig(prompt_templating=prompt_template)
config = OrchestrationConfig(modules=module_config)
orchestration_service = OrchestrationService(config=config)
result = orchestration_service.run(placeholder_values={"user_query": "Who was the first prime minister of india?"})
